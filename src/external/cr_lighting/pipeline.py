...
from cr_lighting.utils.resolution_util import optimal_resize
from cr_lighting.utils.general import round_32, uninvert, invert, get_brightness, to2np
from cr_lighting.utils.color_util import batch_rgb2iuv, batch_iuv2rgb

from cr_lighting.ordinal_util import base_resize, equalize_predictions

from cr_lighting.altered_midas.midas_net import MidasNet
from cr_lighting.altered_midas.midas_net_custom import MidasNet_small
...
def load_models(path, stage=4, device='cuda', compiled=False, chroma_dpt=False, alb_residual=False, checkpoint_dir=None):
    ...
    params:
        ...
        checkpoint_dir (str) optional: directory containing local checkpoint files (for bundled models)
    ...
    if isinstance(path, str):
        if path in ['paper_weights', 'rendered_only', 'v2']:
            raise ValueError(
                f"❌ Version '{path}' is no longer supported in cloud version.\n"
                f"Creative Relight Cloud only supports 'v2.1' with local checkpoints.\n"
                f"Please ensure models are downloaded from HuggingFace first."
            )
        elif path == 'v2.1':
            print("loading v2.1 weights")
            
            # Check if we have local checkpoints
            if checkpoint_dir and FilePath(checkpoint_dir).exists():
                checkpoint_path = FilePath(checkpoint_dir)
                # Check if all required files exist
                required_files = ['stage_0_v21.pt', 'stage_1_v21.pt', 'stage_2_v21.pt', 'stage_3_v21.pt', 'stage_4_v21.pt']
                all_files_exist = all((checkpoint_path / f).exists() for f in required_files)
                
                if all_files_exist:
                    print(f"✅ Loading from local checkpoints: {checkpoint_dir}")
                    ord_state_dict = torch.load(str(checkpoint_path / 'stage_0_v21.pt'), map_location=device)
                    iid_state_dict = torch.load(str(checkpoint_path / 'stage_1_v21.pt'), map_location=device)
                    col_state_dict = torch.load(str(checkpoint_path / 'stage_2_v21.pt'), map_location=device)
                    alb_state_dict = torch.load(str(checkpoint_path / 'stage_3_v21.pt'), map_location=device)
                    dif_state_dict = torch.load(str(checkpoint_path / 'stage_4_v21.pt'), map_location=device)
                else:
                    raise FileNotFoundError(
                        f"❌ Local checkpoint directory exists but files are missing.\n"
                        f"Expected files: {required_files}\n"
                        f"Directory: {checkpoint_dir}\n"
                        f"Please download models first or check your configuration."
                    )
            else:
                raise FileNotFoundError(
                    f"❌ Checkpoint directory not found: {checkpoint_dir}\n"
                    f"Models must be downloaded before loading.\n"
                    f"Please use the model downloader or check your configuration."
                )
